...
class SRSolver(BaseSolver):
    ...
    def feed_data(self, batch, need_HR=True):
        ...
        self.LR3.resize_(input3.size()).copy_(input3)
    ...
    def train_step(self):
        ...
        loss_batch = 0.0
        sub_batch_size = int(self.LR1.size(0) / self.split_batch)
        for i in range(self.split_batch):
            loss_sbatch = 0.0
            split_LR1 = self.LR1.narrow(0, i*sub_batch_size, sub_batch_size)
            split_LR2 = self.LR2.narrow(0, i*sub_batch_size, sub_batch_size)
            split_LR3 = self.LR3.narrow(0, i*sub_batch_size, sub_batch_size)
            split_HR = self.HR.narrow(0, i*sub_batch_size, sub_batch_size)
            if self.use_cl:
                outputs = self.model(split_LR1,split_LR2,split_LR3)
                ...
                output = self.model(split_LR1,split_LR2,split_LR3)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.self_ensemble and not self.is_train:
                ...
                SR = forward_func(self.LR1, self.LR2, self.LR3)
            SR = forward_func(self.LR1, self.LR2, self.LR3)
    ...
    def _overlap_crop_forward(self, x1, x2, x3, shave=10, min_size=100000, bic=None):
        """
        ...
        lr_list2 = [
            x2[:, :, 0:h_size, 0:w_size],
            x2[:, :, 0:h_size, (w - w_size):w],
            x2[:, :, (h - h_size):h, 0:w_size],
            x2[:, :, (h - h_size):h, (w - w_size):w]]
        lr_list3 = [
            x3[:, :, 0:h_size, 0:w_size],
            x3[:, :, 0:h_size, (w - w_size):w],
            x3[:, :, (h - h_size):h, 0:w_size],
            x3[:, :, (h - h_size):h, (w - w_size):w]]
        if w_size * h_size < min_size:
            sr_list = []
            for i in range(0, 4, n_GPUs):
                lr_batch1 = torch.cat(lr_list1[i:(i + n_GPUs)], dim=0)
                lr_batch2 = torch.cat(lr_list2[i:(i + n_GPUs)], dim=0)
                lr_batch3 = torch.cat(lr_list3[i:(i + n_GPUs)], dim=0)
                
                sr_batch_temp = self.model(lr_batch1,lr_batch2,lr_batch3)
               
                if isinstance(sr_batch_temp, list):
                    sr_batch = sr_batch_temp[-1] #get the last ouytput of the list
                else:
            ...
            lr_list = [
                        [x1[:, :, 0:h_size, 0:w_size],            x2[:, :, 0:h_size, 0:w_size],            x3[:, :, 0:h_size, 0:w_size]], 
                        [x1[:, :, 0:h_size, (w - w_size):w],      x2[:, :, 0:h_size, (w - w_size):w],      x3[:, :, 0:h_size, (w - w_size):w]],
                        [x1[:, :, (h - h_size):h, 0:w_size],      x2[:, :, (h - h_size):h, 0:w_size],      x3[:, :, (h - h_size):h, 0:w_size]],
                        [x1[:, :, (h - h_size):h, (w - w_size):w],x2[:, :, (h - h_size):h, (w - w_size):w],x3[:, :, (h - h_size):h, (w - w_size):w]]
                        ]
            sr_list = [
                self._overlap_crop_forward(patch[0], patch[1], patch[2], shave=shave, min_size=min_size) \
    ...
    def get_current_visual(self, need_np=True, need_HR=True):
        ...
        out_dict['LR3'] = self.LR3.data[0].float().cpu()
        out_dict['SR'] = self.SR.data[0].float().cpu()
        if need_np:  out_dict['LR1'], out_dict['LR2'],  out_dict['LR3'], out_dict['SR'] = util.Tensor2np([out_dict['LR1'], out_dict['LR2'], out_dict['LR3'], out_dict['SR']], self.opt['rgb_range'])
        if need_HR:
            out_dict['HR'] = self.HR.data[0].float().cpu()
            if need_np: out_dict['HR'] = util.Tensor2np([out_dict['HR']], self.opt['rgb_range'])[0]
        return out_dict
